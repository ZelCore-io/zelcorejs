...
it('VBK - import Nodecore privatekey', () => {
  ...
  const keyPair = zeltrezjs.veriblockcrypto.KeyPair.importFromNodecorePrivateKey(privateNodecoreKey);
...
it('VBK - export to Nodecore', () => {
  ...
  const priv = zeltrezjs.veriblockcrypto.PrivateKey.fromStringHex(privHex);

  const kp = zeltrezjs.veriblockcrypto.KeyPair.fromPrivateKey(priv);
